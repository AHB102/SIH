...
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QPushButton, QLabel, QStackedWidget, \
    QSplitter, QSizePolicy, QHBoxLayout, QFrame
...
from RedisCache_module import setRedis, delRedis
...
class UbuntuHardeningApp(QMainWindow):
    ...
    def update_feature_button_style(self, button):
        ...
        if self.selected_feature_button:
            self.selected_feature_button.setStyleSheet(
                'QPushButton { background-color: #3498db; color: black; border: 1px solid #3498db; border-radius: 5px; padding: 5px; margin: 5px; }')
    ...
    def update_level_button_style(self, button):
        ...
        if button in level_buttons and button != self.selected_level_button:
            if self.selected_level_button:
                self.selected_level_button.setStyleSheet(
                    'QPushButton { background-color: #3498db; color: black; border: 1px solid #3498db; border-radius: 5px; padding: 5px; margin: 5px; }')
            button.setStyleSheet(
                'QPushButton { background-color: #004080; color: white; border: 1px solid #004080; border-radius: 5px; padding: 5px; margin: 5px; }')
    ...
    def closeEvent(self, event):
        # Override closeEvent to perform cleanup before closing the application
        delRedis(67)
        event.accept()
