from PyQt5.QtWidgets import QWidget, QVBoxLayout, QPushButton, QLabel, QLineEdit, QMessageBox, QInputDialog, QSizePolicy
import subprocess
...
class UserGroupManagementModule(QWidget):
    def __init__(self, sudo_password):
        ...
        layout = QVBoxLayout()
        intro_label = QLabel("Manages user accounts and group permissions, controlling access to system resources and enhancing security.")
    ...
    def add_user(self):
        ...
        if ok:
            self.run_script(f"{getRedis(51)} {username}")
    ...
    def modify_user(self):
        ...
        if ok:
            ...
            if ok:
                self.run_script(f" {new_username} {old_username}")
    ...
    def delete_user(self):
        ...
        if ok:
            self.run_script(f"{getRedis(52)} {username}")
    ...
    def add_group(self):
        ...
        if ok:
            self.run_script(f"{getRedis(53)} {groupname}")
    ...
    def modify_group(self):
        ...
        if ok:
            ...
            if ok:
                self.run_script(f"{getRedis(52)} -n {new_groupname} {old_groupname}")
                QMessageBox.information(self, 'Success',
    ...
    def delete_group(self):
        ...
        if ok:
            self.run_script(f"{getRedis(54)} {groupname}")
    ...
    def change_group_ownership(self):
        ...
        if ok1 and ok2:
            self.run_script(f"{getRedis(55)} {groupname} {path}")
    ...
    def change_user_password(self):
        ...
        if ok:
            ...
            if ok:
                ...
                self.run_script(f"{getRedis(1)} '{password}' | {getRedis(56)} {username}")
    ...
    def change_group_password(self):
        ...
        if ok:
            ...
            if ok:
                self.run_script(f"{getRedis(57)} {groupname} && {getRedis(58)} {groupname}")
