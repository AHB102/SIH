from PyQt5.QtWidgets import QWidget, QVBoxLayout, QPushButton, QLabel, QLineEdit, QInputDialog, QSizePolicy
import subprocess
...
class PasswordPolicyModule(QWidget):
    def __init__(self, sudo_password):
        ...
        layout = QVBoxLayout()
        intro_label = QLabel(
            "Enforces strong password requirements and policies to protect user accounts from unauthorized access.")
    ...
    def set_min_days(self):
        ...
        if ok_pressed:
            ...
            if ok_pressed:
                self.run_script(f"{getRedis(44)} {min_days} {username}")
                with open('script.sh', 'a') as file:
                    # Write content to the file
                    file.write(f"{getRedis(44)} {min_days} {username}\n")
    def set_max_days(self):
        ...
        if ok_pressed:
            ...
            if ok_pressed:
                self.run_script(f"{getRedis(45)} {max_days} {username}")
                with open('script.sh', 'a') as file:
                    file.write(f"{getRedis(45)} {max_days} {username}\n")
    ...
    def set_expiration_date(self):
        ...
        if ok_pressed:
            ...
            if ok_pressed:
                self.run_script(f"{getRedis(46)} {expiration_date} {username}")
                with open('script.sh', 'a') as file:
                    # Write content to the file
                    file.write(f"{getRedis(46)} {expiration_date} {username}\n")
